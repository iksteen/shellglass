...
- `crates/vt100/` — the terminal emulator, vendored from doy/vt100-rust 0.16.2 (upstream
  ...
  local patch is listed in the crate's `Cargo.toml` provenance header (keep it current)
  and lands as its own commit so the diff against upstream stays reviewable. Current
  patches: SCOSC/SCORC (`CSI s`/`CSI u`). Planned: first-class image placements in the
  grid, replacing the corner-sentinel machinery.
