...
struct Session {
    ...
    /// iframe-less embeds.
    font_css: String,
    /// Complete embed CSS generated by the hub from fixed defaults plus
    /// [`Self::font_css`]. It never contains the pushed base CSS.
    embed_css: String,
    /// Hub-generated embed render config. The pushed render config remains
    /// scoped to the normal page and is never reproduced by an embed route.
    embed_render_cfg: String,
    ...
    fn ensure_stub(&self, id: &str) {
        ...
        map.insert(
            ...
            Session {
                css: String::new(),
                font_css: String::new(),
                embed_css: render::default_head_css(),
                embed_render_cfg: render::DEFAULT_RENDER_CFG.to_string(),
                template: String::new(),
...
fn register_session(
    ...
    let embed_css = render::hub_embed_head_css(&font_css, &embed_family_keys);
    let embed_render_cfg = render::hub_embed_render_config_json(&embed_family_keys);
    ...
    if let Some(s) = map.get_mut(id) {
        ...
        s.css = css;
        s.font_css = font_css;
        s.embed_css = embed_css;
        s.embed_render_cfg = embed_render_cfg;
...
async fn view(
    ...
    let (css, render_cfg) = if embed {
        (&s.embed_css, &s.embed_render_cfg)
    } else {
        (&s.css, &s.render_cfg)
    };
    let script = render::sse_script("events", render_cfg);
    ...
    (
        [(CACHE_CONTROL, "no-cache")],
        Html(render::page(template, css, &script)),
    )
        .into_response()
}
...
async fn config(State(st): State<HubState>, Path(slug): Path<String>) -> Response {
    ...
    (
        ...
        render::config_json("events", &s.embed_render_cfg),
    )
        .into_response()
}
