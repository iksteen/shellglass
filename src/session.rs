...
use crate::pty::{self, HeadlessScreen, Notifier, RawMode};
...
/// unix socket for [`attach`]. Returns the frame stream (for the hub) and a
/// [`Notifier`] whose hub up/down events repaint any attached client.
pub fn start_detached(
    ...
    let notifier = {
        let down_tx = core_tx.clone();
        let up_tx = core_tx.clone();
        Notifier::from_fns(
            move |m: &str| {
                let _ = down_tx.send(Core::HubDown(m.to_string()));
            },
            move || {
                let _ = up_tx.send(Core::HubUp);
            },
        )
    };
