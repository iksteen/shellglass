...
#![warn(missing_docs)]
// shellglass: upstream's `#![warn(clippy::cargo)]` dropped — cargo-metadata
// lints are meaningless for a vendored, unpublished workspace member (they
// inspect the host workspace's packages, not this crate).
...
pub use screen::{
    MouseProtocolEncoding, MouseProtocolMode, PlaceOpts, Screen,
};
